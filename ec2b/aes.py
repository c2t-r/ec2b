...
def xor_round_key(state: list, schedule: list, rnd: int):
    for i in range(16):
        state[i] ^= schedule[rnd * 16 + i]

def sub_bytes_inv(a: list, n: int):
    for i in range(n):
...
def mix_cols_inv(state: list):
    for i in range(0, 16, 4):
        col = state[i:i+4]
        mix_col_inv(col)
        state[i:i+4] = col

def oqs_mhy128_enc_c(plaintext: bytes, schedule: list) -> bytes:
    if len(plaintext) != 16 or len(schedule) != 176:
        raise ValueError("Invalid input lengths")

    state = list(plaintext)
    xor_round_key(state, schedule, 0)
    for rnd in range(1, 10):
        sub_bytes_inv(state, 16)
        shift_rows_inv(state)
        mix_cols_inv(state)
        xor_round_key(state, schedule, rnd)
    sub_bytes_inv(state, 16)
    shift_rows_inv(state)
    xor_round_key(state, schedule, 10)
    return bytes(state)
